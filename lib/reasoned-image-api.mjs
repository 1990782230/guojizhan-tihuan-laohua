...
export async function reasonedAnalyzeImages({
  prompt,
  images,
  ...
  if (!prompt?.trim()) throw new Error('检测 Prompt 为空');
  if (!Array.isArray(images) || images.length === 0) throw new Error('检测至少需要一张输入图片');
  ...
  try {
    ...
    if (error?.name === 'AbortError') throw new Error(`推理检测请求超时（>${Math.round(timeoutMs / 1000)}秒）`);
    throw new Error(`推理检测网络请求失败：${error.message}`);
  } finally {
    clearTimeout(timer);
  }

  const bodyText = await response.text();
  if (!response.ok) throw new Error(`Responses检测失败：${extractErrorMessage(response.status, bodyText)}`);

  let data;
  try {
    data = JSON.parse(bodyText);
  } catch {
    throw new Error('Responses检测接口返回了无效JSON');
  }
  const text = data.output_text || (data.output || [])
    ...
    .join('\n')
    .trim();
  if (!text) throw new Error('Responses检测接口没有返回文字结果');
  return { text, responseId: data.id || null, responseModel: data.model || reasoningModel, usage: data.usage || null };
}
